  ...
  type = map(object({
    location = object({
      device_group = optional(object({
        ...
        panorama_device = optional(string) # Panorama device
      })),
      from_panorama_shared = optional(bool) # Located in shared in the config pushed from Panorama
      from_panorama_vsys = optional(object({
